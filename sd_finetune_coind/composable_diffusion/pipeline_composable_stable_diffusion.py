"""
    modified based on https://github.com/energy-based-model/Compositional-Visual-Generation-with-Composable-Diffusion-Models-PyTorch/blob/main/composable_diffusion/composable_stable_diffusion/pipeline_composable_stable_diffusion.py
"""
import inspect
import warnings
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = latents.repeat_interleave(prompt_len+1, dim=0) if do_classifier_free_guidance else latents
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=text_embeddings).sample
                
                # perform guidance
                if do_classifier_free_guidance:
                    each_repeat_noise_pred = noise_pred.chunk(num_images_per_prompt, dim=0)
                    new_noise_pred = []
                    for i in range(num_images_per_prompt):
                        new_noise_pred.append(each_repeat_noise_pred[i][:1] + (weights * (each_repeat_noise_pred[i][1:] - each_repeat_noise_pred[i][:1])).sum(dim=0, keepdims=True))
                    noise_pred = torch.cat(new_noise_pred, dim=0)
