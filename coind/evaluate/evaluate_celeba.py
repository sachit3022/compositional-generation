...
def gender_not_query(y,null_token,guidance_scale):
    B,Q = y.size()
    assert Q == 2
    query = null_token.repeat(2,1).to(dtype=y.dtype,device=y.device)
    query = query.reshape(B,2,2)
    query[:,0,1] = y[:,1]
    query[:,1,0] = y[:,0]==0
    guidance_scale = guidance_scale*3
    guidance_scale = torch.tensor([guidance_scale,-1*guidance_scale]).to(query.device)
    return query,guidance_scale

def smile_not_query(y,null_token,guidance_scale):
    B,Q = y.size()
    assert Q == 2
...
class GenderNotquery(ModelWrapper):
    def prepare_query(self,y,null_token,guidance_scale):
        return gender_not_query(y,null_token,guidance_scale)
class SmileNotquery(ModelWrapper):
    def prepare_query(self,y,null_token,guidance_scale):
        return smile_not_query(y,null_token,guidance_scale)
...
def main(cfg):
    ...
    cs_evaluations = {"val":{
                ...
                "gender_not": {"sampler":CondDDIMPipeline(unet=GenderNotquery(model), scheduler=scheduler),"query":gender_not_query,"metric":CS(classifier = classifier).to(device)},
                "smile_not":{"sampler":CondDDIMPipeline(unet=SmileNotquery(model), scheduler=scheduler),"query":smile_not_query,"metric":CS(classifier = classifier).to(device)}
            },
            "train":{
                "and": {"sampler":CondDDIMPipeline(unet=ANDquery(model), scheduler=scheduler),"query":prepare_and_query,"metric":CS(classifier = classifier).to(device)},
                "joint": {"sampler":CondDDIMPipeline(unet=CFGquery(model), scheduler=scheduler),"query":prepare_and_query,"metric":CS(classifier = classifier).to(device)},
